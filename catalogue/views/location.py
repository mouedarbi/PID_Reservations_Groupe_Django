...
def index(request):
    ...
    title = _('Liste des lieux de spectacle')
...
def show(request, location_id):
    try:
        ...
        raise Http404(_('Lieu inexistant'))
    
    title = _("Fiche d'un lieu de spectacle")
...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Lieu créé avec succès."))
...
def edit(request, location_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Lieu modifié avec succès."))
...
def delete(request, location_id):
    ...
    if request.method == 'POST':
        location.delete()
        messages.success(request, _("Lieu supprimé avec succès."))
        return redirect('catalogue:location-index')
